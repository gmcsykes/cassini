...
from events
inner join targets 
  on targets.id = target_id;select 
    targets.description as target,
    100.0 * 
    (
      (count(1) over (
  ...
  from events
  inner join targets 
    on targets.id = target_id;selectselect 
      targets.description as target,
      100.0 * 
      (
        (count(1) over (
...
date_part('year', time_stamp) as year,
  ...
  from cda.impacts
  where x_velocity <> -99.99, speeds as 
  (select kms.*,(v_kms * 60 * 60)::integer as kmh,
  (v_kms * 60 * 60 * .621)::integer as mph
  from kms
), rollup as (select distinct(year,month),year, month,
max(mph) over (partition by month),
min(mph) over (partition by month)
from speeds GROUP BY
order by year, month)
...
date_part('year', time_stamp) as year,
  ...
  where x_velocity <> -99.99), speeds as
  (select kms.*,(v_kms * 60 * 60)::integer as kmh,
  (v_kms * 60 * 60 * .621)::integer as mph from kms),
  rollup as (select distinct(year,month),year, month,
max(mph) over (partition by month),
min(mph) over (partition by month)
from speeds
